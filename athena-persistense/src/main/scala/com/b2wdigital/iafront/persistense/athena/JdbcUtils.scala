...
import com.b2wdigital.iafront.persistense.athena.util.NextIterator
...
object JdbcUtils extends Logging {
  ...
  def saveTable(
    ...
    val repartitionedDF:DataFrame = options.numPartitions match {
    ...
    repartitionedDF.foreachPartition( (iterator:Iterator[Row]) => {
      val result = savePartition(getConnection, table, iterator, rddSchema, insertStmt, batchSize, dialect, isolationLevel)
    })
  }
